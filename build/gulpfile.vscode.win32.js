...
function buildWin32Setup(arch, target) {
	...
	return cb => {
		...
		const definitions = {
			...
			AppPublisher: product.win32PublisherName || product.companyName || product.nameLong,
			AppPublisherURL: product.win32PublisherUrl || product.reportIssueUrl || '',
			AppSupportURL: product.win32SupportUrl || product.reportIssueUrl || '',
			AppUpdatesURL: product.win32UpdatesUrl || product.win32PublisherUrl || '',
			SetupBaseFilename: product.win32SetupBaseFilename || `${product.nameShort}Setup`,
			Arch: arch,
